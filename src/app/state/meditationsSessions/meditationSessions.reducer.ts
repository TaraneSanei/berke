...
import { AddMeditationSession, AddMeditationSessionFailure, AddMeditationSessionSuccess, UpdateMeditationSession, UpdateMeditationSessionFailure, UpdateMeditationSessionSuccess, LoadMeditationSession, LoadMeditationSessionFailure, LoadMeditationSessionSuccess } from "./meditationSessions.actions";
...
export const meditationSessionsReducer = createReducer(
    ...
    on(UpdateMeditationSession, (state) => ({
        ...state,
        status: 'loading' as 'loading'
    })),
    on(UpdateMeditationSessionSuccess, (state, { session }) => ({
        ...state,
        meditationSessions: state.meditationSessions.map((s) => (s.id === session.id ? session : s)),
        status: 'success' as 'success'
    })),
    on(UpdateMeditationSessionFailure, (state, { error }) => ({
        ...state,
        error,
        status: 'error' as 'error'
    })),
